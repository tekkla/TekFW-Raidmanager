...
final class SubscriptionController extends Controller
{
 ...
 =>  =>  =>  =>  =>  => // Build delete button
 =>  =>  =>  =>  =>  => $actionbarcreateButton('user')->setIcon('user')->setTitle($this->txt('raid_signon_change'))->setRoute('raidmanager_subscription_edit', array(
 =>  =>  =>  =>  =>  =>  =>  => 'id_raid' => $id_raid
 =>  =>  =>  =>  =>  => ));
 ...
 =>  =>  =>  => ->setAjaxTarget('#raidmanager_subscriptions');
 =>  => }
 ...
 =>  =>  =>  => $id_raid = $this->model->hasData() ? $this->model->dataid_raid : $this->model->getRaidId($id_subscription);
 ...
 =>  =>  =>  => $actionbarcreateButton('cancel')->setRoute($route, array(
 =>  =>  =>  =>  =>  => 'id_raid' => $id_raid
 =>  =>  =>  => ))->setTarget($target);
 ...
 =>  =>  =>  => $actionbarcreateButton('save')->setForm($form->getId())->setRoute('raidmanager_subscription_save', $param);
 ...
 =>  =>  =>  => {
 =>  =>  =>  =>  =>  => ->run('Enrollform', array(
 =>  =>  =>  =>  =>  =>  =>  => 'id_subscription' => $this->model->dataid_subscription,
 =>  =>  =>  =>  =>  =>  =>  => 'state' => $this->model->datastate,
 =>  =>  =>  =>  =>  =>  =>  => 'from' => $from,
 =>  =>  =>  =>  =>  =>  =>  => 'id_player' => $this->model->dataid_player,
 =>  =>  =>  =>  =>  =>  =>  => 'id_raid' => $id_raid
 =>  =>  =>  =>  =>  => ));
 =>  =>  =>  =>  =>  => return;
 =>  =>  =>  => }
 ...
 =>  =>  =>  => ->getController('Subscription')ajax($from == 'subscription' ? 'Edit' : 'Index', $param, '#raidmanager_subscriptions');

 =>  =>  =>  => // refresh commentlist
 =>  =>  =>  => ->getController('Comment')ajax('Index', $param, '#raidmanager_comments');

 =>  =>  =>  => // refresh setups only if the player subscribes or unsubscribes
 =>  =>  =>  => if ($this->model->datastate != 0)
 =>  =>  =>  =>  =>  => ->getController('Setup')ajax('Complete', $param, '#raidmanager_setups');
 =>  => }
 ...
 =>  =>  =>  => // cancel button
 =>  =>  =>  => $actionbar = new Actionbar();
 =>  =>  =>  => $actionbarcreateButton('cancel')->setRoute('raidmanager_subscription_index', array(
 =>  =>  =>  =>  =>  => 'id_raid' => $id_raid
 =>  =>  =>  => ));
